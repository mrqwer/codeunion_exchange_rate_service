...
CURRENCIES = ['AUD', 'AZN', 'AMD', 'BYN', 'BRL', 'HUF', 'HKD', 'GEL', 'DKK', 'AED', 'USD', 'EUR', 'INR', 'IRR', 'CAD', 'CNY', 'KWD', 'KGS', 'MYR', 'MXN', 'MDL', 'NOK', 'PLN', 'SAR', 'RUB', 'XDR', 'SGD', 'TJS', 'THB', 'TRY', 'UZS', 'UAH', 'GBP', 'CZK', 'SEK', 'CHF', 'ZAR', 'KRW', 'JPY']
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-'+Behaviour.SHOW_SHORT, '--'+Behaviour.SHOW, action='store_true', help="To get the currency or currencies")
        parser.add_argument('-'+Behaviour.STORE_SHORT, '--'+Behaviour.STORE, action="store_true", help="To store or to update the currencies")
    ...
    def handle(self, *args, **kwargs):
        ...
        if currency_ids:
            ...
            currency_names = map(lambda name: name.upper(), currency_names)
        ...
        if show_data and store_data:
            ...
            if (currency_values):
                self.stdout.write(self.style.NOTICE(f"There is no need to provide values with -{Behaviour.SHOW_SHORT}, --{Behaviour.SHOW}"))
            ...
            if not currencies:
                self.stdout.write(self.style.SUCCESS("There is no data in database. Firstly, update the currencies in database"))
                return
            print("ID    NAME    RATE")
            ...
            if currency_ids:
                if len(currency_ids) != len(currency_values):
                    self.stdout.write(self.style.NOTICE("Currency ids and currency values lists should be equal length"))
                    return
                if len(currency_ids) != len(set(currency_ids)):
                    self.stdout.write(self.style.NOTICE("Please, with 'store' behaviour, there should not be duplicates in 'id' list"))
                    return
                ...
                if len(currency_names) != len(set(currency_names)):
                    self.stdout.write(self.style.NOTICE("Please, with 'store' behaviour, there should not be duplicates in 'names' list"))
                    return
                for c_name, rate in zip(currency_names, currency_values):
                    currency = Currency.objects.get(name=c_name)
                    currency.rate = rate
                    currency.save()
                self.stdout.write(self.style.NOTICE("Names and corresponding rate values are saved successfully"))
                return
            if currency_values:
